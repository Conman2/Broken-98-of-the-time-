...
#Being able to shoot diagonally between blocks, and inbetween them
...
#Sheild morphing through Blocks
...
npc3 = {'Radius': 20, 'Speed': 4, 'Colour': magenta, 'Health': 300, 'Bullet_Damage': 7, 'Sheild_Damage': 2, 'Bullet_rate':10, 'Shoot_range':200, 'Bullet_Speed':6, 'Bullet_Spray':0.3,'Melee_Damage':2,'Block_Damage':0.5} #Doc
...
''' Collosions
def Collosion(Block_size, Radius, x, y, xspeed, yspeed):
    if (block.x - Radius <= x <= block.x + Block_size + Radius) and (block.y - Radius <= y <= block.y + Block_size + Radius):
        if xspeed > 0:
            x = block.x - Radius
            xspeed = -xspeed
        elif xspeed < 0:
            x = block.x + Block_size + Radius
            xspeed = -xspeed
    if (block.x - Radius <= x <= block.x + Block_size + Radius) and (block.y - Radius <= y <= block.y + Block_size + Radius):
        if yspeed > 0:
            y = block.y - Radius
            ...
            y = block.y + Block_size + Radius
            yspeed = -yspeed
    return x, y, xspeed, yspeed
    '''

def Collosion(Block_size, Radius, x, y, block_x, block_y, xspeed, yspeed):
    diffx = x - max(block_x - abs(xspeed)*0.5, min(x, block_x + Block_size + abs(xspeed)*0.5))
    diffy = y - max(block_y - abs(yspeed)*0.5, min(y, block_y + Block_size + abs(yspeed)*0.5))
    if (diffx*diffx + diffy*diffy) < Radius*Radius:
        if x < block_x - abs(xspeed)*0.1:
            xspeed = -xspeed
        elif x > block_x + Block_size + abs(xspeed)*0.1:
            xspeed = -xspeed
        else:
            yspeed = - yspeed
...
while True:
    ...
    for player_bullet in Player_Bullet_array:
        ...
        for block in Block_array:
            player_bullet.Velocity_x,player_bullet.Velocity_y = Collosion(Block_size, player_bullet.Bullet_rad, player_bullet.x, player_bullet.y, block.x, block.y, player_bullet.Velocity_x, player_bullet.Velocity_y)
    ...
    for npc_1 in NPC_1_array:
        ...
        if freeze is False:
            ...
            for block in Block_array:
                npc_1.Velocity_x,npc_1.Velocity_y = Collosion(Block_size, npc_1.Radius, npc_1.x, npc_1.y, block.x, block.y, npc_1.Velocity_x, npc_1.Velocity_y)
            ...
            if BallCollosion(player_rad, npc_1.Radius, npc_1.x, npc_1.y, player_x, player_y) is True:
    ...
    for npc_bullet in NPC_Bullet_array:
        ...
        #Rebounds with Blocks
        for block in Block_array:
            npc_bullet.Velocity_x,npc_bullet.Velocity_y = Collosion(Block_size, npc_bullet.Bullet_rad, npc_bullet.x, npc_bullet.y, block.x, block.y, npc_bullet.Velocity_x, npc_bullet.Velocity_y)
    ...
    for block in Block_array:
        ...
        xspeed, yspeed = Collosion(Block_size, player_rad, player_x, player_y, block.x, block.y, xspeed, yspeed)
